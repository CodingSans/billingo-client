...
export const partnerApiFactory = (instance: AxiosInstance): IPartnerApi => {
  return {
    list: () => instance.get<PartnerList>("/partners").then((r) => r.data),
    create: (partner: PartnerUpsert) =>
      instance.post<Partner>("/partners", partner).then((r) => r.data),
    get: (id: number) =>
      instance.get<Partner>(`/partners/${id}`).then((r) => r.data),
    update: (id: number, partner: PartnerUpsert) =>
      instance.put<Partner>(`/partners/${id}`, partner).then((r) => r.data),
    delete: (id: number) =>
      instance.delete<void>(`/partners/${id}`).then((r) => r.data),
  };
};
