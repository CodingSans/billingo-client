...
export const documentApiFactory = (instance: AxiosInstance): DocumentApi => {
  return {
    ...
    create: (document: DocumentInsert) => instance.post<Document>('/documents', document).then((r) => r.data),
    get: (id: number) => instance.get<Document>(`/documents/${id}`).then((r) => r.data),
    cancel: (id: number) => instance.post<Document>(`/documents/${id}/cancel`).then((r) => r.data),
    createFromProforma: (id: number) =>
      instance.post<Document>(`/documents/${id}/create-from-proforma`).then((r) => r.data),
    download: (id: number) => instance.get<string>(`/documents/${id}/download`).then((r) => r.data),
    onlineSzamla: (id: number) =>
      instance.get<OnlineSzamlaStatus>(`/documents/${id}/online-szamla`).then((r) => r.data),
    listPaymentHistory: (id: number) => instance.get<PaymentHistory[]>(`/documents/${id}/payments`).then((r) => r.data),
    ...
    publicUrl: (id: number) => instance.get<DocumentPublicUrl>(`/documents/${id}/public-url`).then((r) => r.data),
    send: (id: number, send: SendDocument) =>
      instance.post<SendDocument>(`/documents/${id}/send`, send).then((r) => r.data),
  };
};
